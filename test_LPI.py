from learning_policies.learning_policies import AgentsManager, LPIAgent
...
agents_LPI = [LPIAgent(config, agents_manager, i) for i in range(num_agents)]
...
print(states[0])
print(get_combined_observation_for_LPI(states[0], agents_LPI[agents[0]].neighbours_beta))
for i in range(len(states) - 1):
    # policy evaluation
    agent = agents_LPI[agents[i]]
    next_agent = agents_LPI[agents[i + 1]]
    observation = get_combined_observation_for_LPI(states[i], agents_LPI[agents[i]].neighbours_beta)
    next_observation = get_combined_observation_for_LPI(s_prime[i], agents_LPI[agents[i + 1]].neighbours_beta)
